...
1. **Profiler & Qualitatives Audit:** Das Plugin agiert als KI-Interviewer, analysiert unstrukturierte Prozessbeschreibungen und identifiziert asynchrone Flaschenhälse (Low-Hanging Fruits), um sofortiges Vertrauen und messbaren ROI aufzuzeigen. 
    *   *Highlight-Szenario (Data/Lead Arbitrage):* Ein starker "Quick Win" ist die Umleitung ineffizienter Budgets. Gibt der Kunde beispielsweise 1.000 € für Meta-Ads mit geringer Conversion aus, spezifiziert das System automatisiert eine alternative, datengetriebene Outbound-Pipeline (z.B. Web Scraping via [Apify](https://apify.com) gekoppelt mit LLM-Personalisierung). Dies liefert massiv höhere Lead-Volumina zu einem Bruchteil der Ad-Kosten.
...
3. **Audit & Architektur:** Erst nach ROI-Nachweis erfolgt der lesende Systemzugriff (via Composio MCP) auf dezidierte, vom Kunden angelegte *Parallelinstanzen/Sandboxes*. Dies ermöglicht den Entwurf von KI-Systemen, die neben Legacy-Prozessen laufen und ein Höchstmaß an Sicherheit und Compliance (DACH) oder Skalierungsgeschwindigkeit (Russland/Indonesien) garantieren. Im Kern entwirft das Plugin eine **Data-Centric Architecture** für den Kunden: Daten werden aus App-Silos befreit und durch mehrschichtige Sicherheits- und Governance-Layer (Zero Trust) geschützt, sodass KI-Agenten sicher andocken können.
...
- **Data-Centric Security by Design:** Wir verkaufen keine riskanten API-Integrationen in Legacy-Systeme. Stattdessen positionieren wir eine entkoppelte, mehrschichtige Architektur (Core Data, Governance, Security, Semantic API), die es KMUs erlaubt, autonome KI-Agenten bei absolutem "Zero-Trust"-Schutz und vollautomatisierter Compliance (z.B. DSGVO, 152-ФЗ) einzusetzen.
...
- **C-Level Entscheider (Geschäftsführer/COO):** Fokus auf strategischen ROI. Sie sind die *Empfänger* der erstellten Stakeholder-Berichte. Ihr Erlebnis ist geprägt von der unerwarteten Aktualität der Lösung und dem beeindruckenden Format der Delivery (interaktive HTML-Dashboards im Corporate Design). Gerade in Emerging Markets wie Indonesien hilft die Session, den massiven "Readiness-Gap" (hohes Vertrauen in KI, aber extrem geringe Inhouse-Skills) zu überbrücken.
...
3. **Phase 3: Formaler Abschluss & Setup (Legal Tech & Compliance)**
   - *Aktion:* Nach Angebotsrücklauf orchestriert das Plugin die rechtlichen Leitplanken. Es nutzt verifizierte Legal-Tech MCPs (z.B. Ironclad, eSignatures, oder regionale Lösungen wie Law7 für RU, und Indonesian Legal MCP für ID) zur Vertragsgenerierung.
   - *Liability & HITL:* Einbindung strikter "Human-in-the-Loop" (HITL) Freigabeprozesse und Haftungs-Disclaimers (User Assumption of Risk). Verträge werden erst nach menschlichem Audit über eSignature/DocuSign MCP versendet.
   - *Artefakte:* Maßgeschneiderte **Verträge** und **NDAs** (geprüft auf regionale Regularien wie DSGVO, 152-ФЗ oder UU PDP), Initialisierung der `AI_Seed.txt`, Einrichtung des Private GitHub Repo und GCC-Memorys.

4. **Phase 4: Remote Discovery Session (Der Kernprozess)**
   - *Aktion:* Durchführung der Session basierend auf BMAD-Methodik.
   - *Sub-Aktion 1 (Qualitativ):* Remote Interviews mit Stakeholdern. Auswertung von unstrukturierten Daten (Fragebögen, Prozessdokumente, CRM-Exporte, Transkripte) zur Identifikation von "Low-Hanging Fruits". Berücksichtigung marktspezifischer Kommunikationswege (z.B. Dominanz von WhatsApp Business API in Indonesien).
   - *Sub-Aktion 2 (Quantitativ/System):* Lesende Analyse von digitalen IT-Systemen (CRM, ERP, Projektmanagement). Hierbei orchestriert das Plugin je nach Zielmarkt die passenden Konnektoren (z.B. DACH: Salesforce/HubSpot; RU: Bitrix24/amoCRM; ID: Mekari Qontak, Barantum, Odoo) auf speziellen Kunden-Parallelinstanzen via Composio oder nativen MCPs. Relevante Datenlokalisierungsvorgaben (wie inländische Server für OJK-regulierte indonesische Kunden nach UU PDP) werden im Audit geprüft.
   - *Leitmotiv:* Jeder identifizierte Quick Win wird als Baustein in die große Vision einer datenzentrischen Architektur eingeordnet. Ein starker Fokus liegt auf radikalen ROI-Hebeln: Substitution ineffizienter Paid-Ads durch hyper-personalisierte AI-Scraping-Pipelines (Apify). Für die strategische und textliche Ausführung dieser LeadGen-Pipelines wird das Open-Source Framework [Marketing Skills for AI Agents](https://github.com/coreyhaines31/marketingskills) herangezogen, welches bewährte Agenten-Workflows für Conversion-Optimierung (CRO), Programmatic SEO und Cold-E-Mailing nativ bereitstellt.
...
5. **Phase 5: Delivery & Follow-Up (Architektur & Folgetikett)**
   ...
   - *Artefakte:* **Architektur- und Lösungsspezifikationen** (inkl. Web-Applikationsplänen), die konsequent einem **Data-Centric Architecture** Modell folgen (inklusive definierter Layer für Data Governance und Zero-Trust Security). **Implementierungs-Angebot** für die vorgeschlagenen Lösungen.
...
- **Margen-Ziel (ROI):** Die erstellten Angebote müssen basierend auf dem hochautomatisierten Prozess (EDD/TDD für Delivery) und dem tatsächlichen Realisierungsaufwand konstant eine **Bruttomarge von 200 %** sichern. Kalkulationsbeispiel: 10h netto für die Durchführung der Session × $80 = $800 Kostenbasis → Angebotspreis für den Kunden = $2.400.
- **Corporate Consistency:** 100 % der freigegebenen Architekturdokumente und Berichte entsprechen dem final generierten Corporate Webdesign (`template_presentation.html` / `template_artifact.html`), was den wahrgenommenen Service-Wert im KMU-Sektor drastisch steigert.
...
- **Multi-Markt Connector-Set (Read-Only, v1):** Russland: Bitrix24, kommo/amoCRM, 1C. Indonesien: Odoo, Mekari Qontak, Barantum. Deutschland: über Composio-Verbindungen (konfigurationsgetrieben; Zielsysteme v1: HubSpot, Salesforce, Microsoft 365, Google Workspace). Zudem Implementierung von Legal-Tech MCP Servern (z.B. Ironclad, DocuSign) für Phase 3.
- **Zielmarkt-spezifische MCP-Integrationen (Lesend):** Für dominante KMU-Systeme im DACH-Markt kommen nativ Open-Source MCP-Server zum Einsatz (Salesforce, HubSpot). Für breitere Tool-Landschaften fungiert **Composio MCP** als Bridge. Zusätzlich bindet das System funktionale "Soft-Skills" wie das **"Marketing Skills for AI Agents" Framework** ein, um architektonische Marketing-Lösungen (LeadGen, SEO) mit validierten Prompts auszustatten. Generische REST API Fallbacks bilden ein zusätzliches Sicherheitsnetz.
